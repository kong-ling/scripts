my @testResult = `find bwcSwOdrxProc* -name "*.txt" | xargs ls -rt | xargs grep "Result: F" | cat -n`;

foreach (@testResult)
{
    # ./bwcSwSystemRx_paraCa_03_level1/reports_lingkong/case_113__2017-10-04_02_41_30.txt:Result: PASS
    if(/(\w+)\/(\w+__\d{4}-\d{2}-\d{2}_\d{2}_\d{2}_\d{2})\.txt:Result: PASS/)
    {
        my $testLog = join '/', $1.'_run', $2;
        #print "$testLog\n";
    ...
    elsif(/(\w+)\/(\w+__\d{4}-\d{2}-\d{2}_\d{2}_\d{2}_\d{2})\.txt:Result: FAIL/)
    {
        my $testLog = join '/', $1.'_run', $2;
        print "$testLog\n";
